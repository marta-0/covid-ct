...
def classification_metrics(model, X_val, y_val, f1_metric):
...
#     plt.figure(figsize=(9, 8))
    sns.heatmap(cm, annot=True, xticklabels=CLASS_NAMES, yticklabels=CLASS_NAMES, cbar=False)
...
def print_auc(model, model_name, file_name, X_val, y_val):
...
def calculate_aucs(filename, model_functions, model_functions_rgb, model_bit, X_val, y_val, X_val_rgb, y_val_rgb):
    for model_name, model in zip(['Simple', 'Tiny', 'Small', 'LargeW', 'LargeT'], model_functions):
        print_auc(model, model_name, filename, X_val, y_val)
    
    for model_name, model in zip(['EfficientNetB3-ImageNet', 'EfficientNetB3'], model_functions_rgb):
        print_auc(model, model_name, filename, X_val_rgb, y_val_rgb)
